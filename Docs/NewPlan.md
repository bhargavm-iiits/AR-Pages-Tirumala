...
| 08 | Demo map + live position | **partial** | `MapView` (271 ln), `DemoBasemap`, `RouteOverlay`, `UserMarker`, `PoiMarkerLayer` are all present and functional; `isBridged` waypoints render amber via `RouteOverlay.cs:34,41`. But there is **no `IBasemapSource` interface and no `TileBasemap`** — Rev 2's planned v1.1 tile swap-in point does not exist yet, so `DemoBasemap` is a concrete dependency rather than an interface implementation |
| 09 | Positioning + session logic | **done** | `LocationProvider` (164 ln) drives real `Input.location` with a `FineLocation` runtime-permission flow (`LocationProvider.cs:111-152`); `GpsKalmanFilter`, `TraceReplaySource` (wired in at `LocationProvider.cs:47` for desk-testing without a device), `RouteProgressTracker`, `EtaEstimator`, `LandmarkTriggerService`, `ManeuverDetector`, `NavigationSession` all present |
| **10** | **AR navigation on device — the hard gate** | **⚠ written, entirely device-unverified** | Every AR file compiles and is wired end-to-end: `ARSessionBootstrapper` builds the AR session hierarchy in code, `ARNavigationScreen` reacts to its state machine and constructs `GroundPlacementService` → `HybridLocalizationEngine` → `DynamicArrowManager` on `Ready` (`ARNavigationScreen.cs:107-175`). `GroundPlacementService` implements the full three-tier fallback (Plane → FeaturePoint → fixed-offset Fallback). But `HybridLocalizationEngine` is only 41 lines and materially thinner than Rev 2's "fuse four weak signals" — see §03, D1. Every AR-adjacent class carries an explicit "Device-unverified" note in its own doc comment; none of it has run on a phone |
...
| 14 | Full field test | **blocked** | Rev 2 §11 specifies this phase runs "with the debug overlay recording" — but neither `DebugOverlay` nor `GpsTraceRecorder` exist anywhere in `Assets/Scripts/`. The field test as specified cannot currently be run |
...
|---|---|---|---|---|
| D1 | **high** | AR pose fusion is materially thinner than planned. `HybridLocalizationEngine.FeedFix` only establishes a frame once, then `Nudge()`s it by a flat 6% of positional discrepancy per fix (`GeoAnchorFrame.cs:11`, `NudgeFactor = 0.06f`). There is **no GPS-outlier rejection and no re-seed path if the tracked pose diverges** — a bad fix nudges the frame just like a good one, with nothing to catch runaway drift. Compass heading is captured once at `Establish()` and never corrected afterward, so any magnetometer error present at seed time is permanent for the rest of the session | `HybridLocalizationEngine.cs` (41 lines, whole file); `GeoAnchorFrame.cs:11`, `:59-68` (`Nudge`) | Add an accuracy-scaled outlier gate + a consecutive-rejection counter that forces a re-seed; add slow continuous compass correction of `_headingOffsetDeg` rather than a one-time capture |
| D2 | **high** | No `Diagnostics/DebugOverlay.cs`, no `Diagnostics/GpsTraceRecorder.cs` | absent from `Assets/Scripts/` entirely | Build both — Phase 14's field test is unrunnable without them, and D1's tuning is unverifiable without them |
...
| D9 | low | One hardcoded English string bypasses the localisation layer | `MapScreen.cs:49` — `"Route unavailable"` passed directly to `UIFactory.Label` instead of `Loc.T(...)` | Add a `map.route_unavailable` key, route it through `Loc.T` |
...
| **B** | **Build the instruments** | `Diagnostics/DebugOverlay.cs` (frame drift, GPS residual, active ground-placement tier, fps, thermal state, AR session state — overlay text on-device) and `Diagnostics/GpsTraceRecorder.cs` (1 Hz GeoJSON trace + a per-second diagnostics CSV, matching the pattern used successfully in the sibling `AR_Navigation` project's `SessionTraceRecorder`). Resolves D2. **Gates every phase after this** — without these, a device walk produces impressions, not data. | ~1 day |
| **C** | **Harden the AR pose fusion** | Resolves D1. Add a GPS-outlier reject threshold that scales with the fix's own reported horizontal accuracy; add a consecutive-rejection counter that forces a `GeoAnchorFrame` re-seed once sustained disagreement suggests the tracked pose (not the GPS) has diverged, rather than leaving it permanently stranded; add slow continuous compass correction of `_headingOffsetDeg` instead of a one-time capture at `Establish()`. Expose the rejection count and last-correction source to `DebugOverlay`. Written and reasoned about carefully since none of this code has ever executed against real sensor data. | ~1–2 days |
...
| **E** | **Data + content correctness** | D4 (delete duplicate landmark), D5 (resolve dead audio path), D6 (add `description` field), D9 (route the one hardcoded string through `Loc.T`). Then build `IBasemapSource` as a proper interface with `DemoBasemap` as one implementation and stub out `TileBasemap` as the second, so the eventual tile swap-in (Rev 2's original Phase 08 plan) is an interface substitution rather than a rewrite. Then D8 — bundle real Noto TTFs and produce the four translation JSON files. | ~2–3 days + translation/content time |
